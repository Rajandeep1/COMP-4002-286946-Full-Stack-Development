// AddRoleForm — Lab 5.2
// isPending prop comes from useMutation.isPending in OrganizationPage.
...
interface AddRoleFormProps {
  ...
  createRole: (firstName: string, lastName: string, roleTitle: string) => Promise<LeadershipServiceResult>;
  isPending?: boolean;
}

const AddRoleForm = ({ onSuccess, createRole, isPending = false }: AddRoleFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const result = await createRole(firstName.value, lastName.value, roleTitle.value);
      ...
      <div className="form-inner">
        ...
        <form className="employee-form" onSubmit={handleSubmit} noValidate>
          ...
          <button type="submit" className="form-submit" disabled={isPending}>
            {isPending ? 'Saving…' : 'Add to Organization'}
          </button>
